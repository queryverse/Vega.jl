...
function fix_shortcuts(spec::Dict{Symbol,Any}, positional_key::Symbol)
    ...
    if haskey(new_spec, :encoding)
        new_encoding_dict = Dict{Symbol,Any}()
        for (k,v) in new_spec[:encoding]
            if v isa Symbol
                new_encoding_dict[k] = Dict{Symbol,Any}(:field=>string(v),:type=>"quantitative")
            elseif v isa String
                new_encoding_dict[k] = Dict{Symbol,Any}(parse_shortcut(v)...)   
            else
    ...
    if haskey(new_spec, :data)
        if TableTraits.isiterabletable(new_spec[:data])
            ...
            new_spec[:data] = Dict{Symbol,Any}(:values => recs)
        end
    end
...
macro vlplot(ex...)
    ...
    new_ex = MacroTools.prewalk(ex) do x
        ...
        if x isa Expr && x.head==:(=) && x.args[2] isa Symbol
            return :($(QuoteNode(x.args[1]))=>$(esc(x.args[2])))
        elseif x isa Expr && x.head==:(=)
                return :($(QuoteNode(x.args[1]))=>$(x.args[2]))
    ...
    return :( VegaLite.VLSpec{:plot}(fix_shortcuts($new_ex, $(QuoteNode(positional_key)))) )
end
...
function Base.hcat(A::VLSpec{:plot}...)
    spec = VLSpec{:plot}(Dict{Symbol,Any}())
    spec.params[:hconcat] = []
    for i in A
...
function interactive()
    i -> begin
        i.params["selection"] = Dict{Symbol,Any}()
        i.params["selection"][:selector001] = Dict{Symbol,Any}()
